                    ...
                    if call=="In Play":
                        ...
                        if result=="OUT":
                            ...
                            if result=="HR":
                                ...
                                if runner and R.random()<steal_attempt_probability(runner,strategies[fid]):
...
class H(BaseHTTPRequestHandler):
    ...
    def api_get(self,p):
        ...
        if p in ("/health","/api/health"):
            return self.out({"ok":True,"service":"EBL","version":"1.1.0"})
        ...
        if p.startswith("/api/profile/"):
            ...
            players=[]
            for row in c.execute(
                """SELECT p.id,p.name,p.type,p.primary_pos,p.franchise_id,p.status,p.active,p.season_json,
                          f.name team_name
                ...
                pl["championships"]=[dict(x) for x in c.execute(
                    """SELECT pc.season,pc.franchise_id,f.name team_name
                       ...
                       WHERE pc.player_id=? ORDER BY pc.season DESC""",(pl["id"],)
                )]
                pl["season_history"]=[{**dict(x),"stats":json.loads(x["stats_json"] or "{}")} for x in c.execute(
                    "SELECT season,franchise_id,player_type,stats_json FROM season_history WHERE player_id=? ORDER BY season DESC",
                    (pl["id"],)
                )]
                players.append(pl)
        ...
        if p=="/api/commish/validate-league":
            ...
            team_count=c.execute("SELECT COUNT(*) n FROM franchises").fetchone()["n"]
            ...
            if phase=="REGULAR" and day<81 and next_day_games!=15:
                issues.append(f"Expected 15 scheduled games on Day {day+1}, found {next_day_games}")
            ...
            out={
                ...
                "counts":{
                    "franchises":team_count,"active_players":player_count,"roster_slots":slot_count,
    ...
    def api_post(self,p):
        ...
        if p=="/api/commish/next-season":
            ...
            try:
                ...
                players=c.execute(
                    """
                    SELECT id,franchise_id,type,season_json
                    FROM players
                    """
                ).fetchall()
                ...
                if champion:
                    c.execute(
                        """
                        INSERT OR REPLACE INTO season_champions(season,franchise_id)
                        VALUES(?,?)
                        """,
                ...
                # ---------------------------------------------
                # ARCHIVE FRANCHISE SEASON HISTORY
                # ---------------------------------------------
                for fr in c.execute(
                    "SELECT id,wins,losses,runs_for,runs_against FROM franchises"
                ).fetchall():
                ...
                for pl in players:
                    if pl["type"]=="H":
                        ...
                        new_stats={
                            "G":0,"GS":0,"OUTS":0,"H":0,"ER":0,"BB":0,"SO":0,
                            "W":0,"L":0,"SV":0
                        }
                ...
                c.execute(
                    ...
                    ON CONFLICT(k) DO UPDATE SET v='REGULAR'
                    """
                )
                c.execute(
                    """
